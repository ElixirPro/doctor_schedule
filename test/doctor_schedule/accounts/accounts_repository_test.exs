defmodule DoctorSchedule.Accounts.Repositories.AccountsRepositoryTest do
  ...
  describe "users" do
    ...
    test "list_users/0 returns all users" do
      user = user_fixture()
      assert AccountsRepository.list_users() == [user]
    end

    test "get_user!/1 returns the user with given id" do
      user = user_fixture()
      assert AccountsRepository.get_user!(user.id) == user
    end

    test "create_user/1 with valid data creates a user" do
      assert {:ok, %User{} = user} = AccountsRepository.create_user(@valid_attrs)
    ...
    test "create_user/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = AccountsRepository.create_user(@invalid_attrs)
    end

    test "update_user/2 with valid data updates the user" do
      user = user_fixture()
      assert {:ok, %User{} = user} = AccountsRepository.update_user(user, @update_attrs)
    ...
    test "update_user/2 with invalid data returns error changeset" do
      user = user_fixture()
      assert {:error, %Ecto.Changeset{}} = AccountsRepository.update_user(user, @invalid_attrs)
      assert user == AccountsRepository.get_user!(user.id)
    end
